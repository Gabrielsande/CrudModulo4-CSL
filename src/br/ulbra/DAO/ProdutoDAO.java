...
public class ProdutoDAO extends AbstractDAO {
    ...
    public void salvar(Produto p) throws SQLException {
        String sql = "INSERT INTO produto (nome, preco, idfor) VALUES (?, ?, ?)";
    ...
    public Produto buscarPorId(int id) throws SQLException {
        String sql = "SELECT p.id, p.nome, p.preco, f.idfor as f_idfor, f.nomerazao as f_nomerazao, f.nomefantasia, f.telefone, f.email " +
                     "FROM produto p INNER JOIN fornecedor f ON p.idfor = f.idfor WHERE p.id = ?";
        try (Connection con = getConnection();
    ...
    public List<Produto> listar() throws SQLException {
        String sql = "SELECT p.id, p.nome, p.preco, f.idfor as f_idfor, f.nomerazao as f_nomerazao, f.nomefantasia, f.telefone, f.email " +
                     "FROM produto p INNER JOIN fornecedor f ON p.idfor = f.idfor ORDER BY p.nome";
    ...
    public void atualizar(Produto p) throws SQLException {
        String sql = "UPDATE produto SET nome = ?, preco = ?, idfor = ? WHERE id = ?";
        try (Connection con = getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
